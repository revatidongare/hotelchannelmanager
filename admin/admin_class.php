...
Class Action {
	...
	function save_book(){
		...
		$query = "SELECT * FROM `tbl_events` WHERE `start` BETWEEN '$date_in' AND '$out' AND `end` BETWEEN '$date_in' AND '$out' AND `category` = '$cid'";
			include '../config.php';
			...
			foreach($result as $room){
				$id2 = $room['id'];
				$title1 = $room['title'] - '$noofroom';
				$query2 = "UPDATE `tbl_events` SET `title` = '$title1' WHERE `id` = $id2";
				include '../config.php';
